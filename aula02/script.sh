ASSESTSFOLDER=assets/timeline

# echo $ASSESTSFOLDER

for mediaFile in `ls $ASSESTSFOLDER | grep .mp4`; do

    FILLENAME=$(echo $mediaFile | sed -n 's/.mp4//p' | sed -n 's/-1920x1080//p')

    INPUT=$ASSESTSFOLDER/$mediaFile
    FOLDER_TARGET=$ASSESTSFOLDER/$FILLENAME
    mkdir -p $FOLDER_TARGET

    # Criar arquivos de resoluções diferentes na pasta.
    OUTPUT=$ASSESTSFOLDER/$FILLENAME/$FILLENAME
